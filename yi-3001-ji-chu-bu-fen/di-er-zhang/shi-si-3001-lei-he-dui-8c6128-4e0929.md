...
3. 动态地计算值，getter/setter提供了一些属性读取的封装，可以让代码更便捷，例如存储first\_name和last\_name字段，但是提供full\_name属性的getter
...
1. 输出用户全名\(经典类\)
   ...
   \`\`\`python  
   class Person\(object\):  
       def **init**\(self, first\_name,last\_name,age\):  
...
4. 新式类中的属性有三种访问方式，并分别对应了三个被`@property`、`@方法名.setter`、`@方法名.deleter`修饰的方法；
