...
##### 3.3、新式类（classic class）\(Python2.2-2.3\)
...
5. 第一种，正常继承模式，看起来正常，不过实际上感觉很别扭，比如B明明继承了D的某个属性（假设为name），C中也实现了这个属性name，那么BFS明明先访问B然后再去访问C，但是为什么name这个属性会是C？这种应该先从B和B的父类开始找的顺序，我们称之为单调性。

6. 第二种，棱形继承模式，这种模式下面，BFS的查找顺序虽然解了DFS顺序下面的棱形问题，但是它也是违背了查找的单调性。
